...
export function fmtDateTime(date: Date | number | string, opts?: Intl.DateTimeFormatOptions) {
  const lang = getLang();
...
export function fmtRelative(target: Date | number | string, now: Date | number = Date.now()) {
  const lang = getLang();
